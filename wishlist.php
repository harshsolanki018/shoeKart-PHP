...
$wishlist_items = [];
$stmt = $conn->prepare("SELECT w.id AS wish_id, p.id AS product_id, p.name, p.brand, p.price, p.discount, p.image_link FROM wishlist w JOIN products p ON w.product_id = p.id WHERE w.username = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_id'])) {
    ...
    $del = $conn->prepare("DELETE FROM wishlist WHERE id = ? AND username = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buy_now'])) {
    ...
    if ($stmt->execute()) {
        $conn->query("DELETE FROM wishlist WHERE product_id=$product_id AND username='$user'");
...
<main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-sm font-semibold uppercase tracking-[0.24em] text-blue-600">Wishlist</p>
                <h1 class="mt-2 text-3xl font-bold text-slate-900">Your Wishlist</h1>
                <p class="mt-2 text-sm text-slate-500">Keep a short list of shoes you want to revisit later.</p>
            </div>
            <a href="index.php" class="inline-flex rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Back to shop</a>
        </div>
    ...
    <?php if (count($wishlist_items) === 0): ?>
        <div class="mt-6 rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
            ...
            <p class="mt-2 text-sm text-slate-500">Save products from the catalog to compare them later.</p>
            <a href="index.php" class="mt-6 inline-flex rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-800">Browse shoes</a>
        </div>
    <?php else: ?>
                ...
                <article class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="p-4">
                        ...
                        <div class="mt-4 flex flex-wrap gap-2">
                            <form method="post">
                                <input type="hidden" name="remove_id" value="<?= $item['wish_id'] ?>">
